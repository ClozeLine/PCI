            ...
            if type(other[0]) == PreyAgent:
                if self.pos.distance_to(other[0].pos) <= 20 and not self.on_site():
                    ...
                    for agent in self.in_proximity_accuracy():
                        if type(agent[0]) == PreyAgent:
                            preys += 1
                        elif type(agent[0]) == PredatorAgent:
                            predators += 1

                    reproduction_rate = predators / (predators + preys)
                    ## fix reproduction rate

                    if self.food >= 120 and (random.random() > reproduction_rate):
                        self.food -= 50
                        self.reproduce()
...
data = (
    Simulation(
        ...
        PredatorPreyConfig(image_rotation=True, movement_speed=1, radius=150, seed=1, fps_limit=60, duration=6000)
